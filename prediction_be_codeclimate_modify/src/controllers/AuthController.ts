...
import { operatoreRepository } from "../middlewares/repo";
...
class AuthController {
  static login = async (req: Request, res: Response) => {
    ...
    try {
      operatore = await operatoreRepository.findOneOrFail({ where: { username } });
    } catch (error) {
  ...
  static generateMailOtp = async (param: any) => {
    ...
    return await new Promise(async (resolve, reject) => {
      await operatoreRepository.findOneOrFail({ where: { mail: param.mail } })
        .then(async (res) => {

          res.otp = otpGenerated
          await operatoreRepository.save(res)
  ...
  static rigenerateMailOtp = async (req: Request, res: Response) => {
    const otpGenerated = generateOTP();
    let { mail } = req.body;
    await operatoreRepository.findOneOrFail({ where: { mail: mail } })
      .then(async (op) => {

        op.otp = otpGenerated
        await operatoreRepository.save(op)
          .then(async (newOp: any) => {
            await SenderController.sendEmail({
              to: mail,
              OTP: otpGenerated,
  ...
  static validateOtp = async (req: Request, res: Response) => {
    ...
    await operatoreRepository.findOneOrFail({ where: { mail: operatore.mail } })
      .then((op: any) => {
        if (op.otp !== otp) {
  ...
  static changePassword = async (req: Request, res: Response) => {
    ...
    let operatore: Operatore;
    try {
      operatore = await operatoreRepository.findOneOrFail(id);
    } catch (id) {
      res.status(401).send();
    }
  ...
  static generateTemporaryToken = async (req: Request, res: Response) => {
    ...
    operatoreRepository.findOneOrFail({ where: { username: username } }).then(async operatore => {
  ...
  static checkToken = async (req: Request, res: Response) => {
    ...
    operatoreRepository.findOneOrFail({ where: { username: username.toString() } }).then(async operatore => {
