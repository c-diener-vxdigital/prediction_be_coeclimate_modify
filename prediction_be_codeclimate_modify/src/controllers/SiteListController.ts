...
import { blacklistRepository } from "../middlewares/repo";
...
class BlacklistController {
    ...
    static create = async (req: Request, res: Response) => {
        ...
        if (blacklist.status) {
            await DnsServerController.callApi('POST', { domains: [req.body.url] }).then(async (resp) => {
                await blacklistRepository.save(blacklist).then(bl => {
    ...
    static edit = async (req: Request, res: Response) => {
        ...
        await blacklistRepository.findOneOrFail({ where: { id: idBlacklist } })
            .then(async blacklist => {
                if (blacklist.status && blacklist.url !== req.body.url) {
                    await DnsServerController.callApi('PUT', { old_domain: blacklist.url, new_domain: req.body.url }).then(async (resp) => {
                        ...
                        await blacklistRepository.save(blacklist).then(() => {
    ...
    static delete = async (req: Request, res: Response) => {
        ...
        try {
            ...
            let idBlacklist: any = valueToString(req.query.id);
            let result = [];
            let exception = []
