...
import { decrypt, encrypt, valueToString } from "../middlewares/utils";
import DnsServerController from "./DnsServerController";
import { SiteList } from "../entity/SiteList";
import { blacklistRepository, genitoreRepository, segnalazioneRepository } from "../middlewares/repo";
...
class SegnalazioneController {
    ...
    static create = async (req: Request, res: Response) => {
        ...
        if (req.query.codiceGenitore) {
            ...
            await segnalazioneRepository.save(segnalazione).then((bl: any) => {
                bl = encrypt(bl)
    ...
    static edit = async (req: Request, res: Response) => {
        ...
        await segnalazioneRepository.findOneOrFail({ where: { id: idSegnalazione } })
            .then(async segnalazione => {
                ...
                await segnalazioneRepository.save(segnalazione);
                if (segnalazione.status === 'TERMINATA') {
                    ...
                    await blacklistRepository.save(blacklist).then(async (bl) => {
    ...
    static delete = async (req: Request, res: Response) => {
        ...
        try {
            ...
            let idSegnalazione: any = valueToString(req.query.id);
    ...
    static get = async (req: Request, res: Response) => {
        ...
        try {
            if (req.query.id != undefined) {
                ...
                await genitoreRepository.findOneOrFail({ where: { codice: codiceGenitore } })
                    .then(async (gen) => {
